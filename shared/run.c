...
void print_stats(current_run_state_t *current_run_state)
{
    ...
    for (uint64_t i=0; i < binary_file_details->set_memory_count;i++)
    {
        if (binary_file_details->set_memory[i].length>0)
        {
            ...
            {
                printf_output("Input %li at address: 0x%" PRIx64 " provided:  ",i,binary_file_details->set_memory[i].address);
...
{
    uc_engine *uc_count;
    my_uc_engine_setup(&uc_count,current_run_state);
    ...
    my_uc_hook_add("min_max_mod",uc_count, &hk_min_max_mod, UC_HOOK_CODE, hook_min_max_mod, current_run_state,1,0);
    ...
    //my_uc_hook_del(uc, hk_placebo,current_run_state);
    my_uc_hook_del(uc_count, hk_min_max_mod,current_run_state);
    ...
    printf_output("Total instructions run:   %li\n",num_instr);    
    printf_output("Time to run:              %li ns \n", current_run_state->time_to_run);
    printf_output("Time per instruction:     %li ns \n", tpi);
    my_uc_close(uc_count,current_run_state);
...
void run_to_write_stats(current_run_state_t *current_run_state)
{
    ...
    if (current_run_state->start_from_checkpoint==1)
    {
        ...
        fprintf(fd, " >> If this fails - consider setting 'number of checkpoints' to a lower number than: %li.\n",current_run_state->total_num_checkpoints); 
    ...
    printf_output("Total number of checkpoints created: %li (requested: %li). \n",valid_checkpoint_count,current_run_state->total_num_checkpoints); 
    current_run_state->total_num_checkpoints=valid_checkpoint_count;

    uc_engine *uc_stats;
    my_uc_engine_setup(&uc_stats,current_run_state);

    uc_hook hk_stats;

    // Add hook
    my_uc_hook_add("hook_code_stats",uc_stats, &hk_stats, UC_HOOK_CODE, hook_code_stats, current_run_state, 1,0);
    // go
    my_uc_engine_start(uc_stats, current_run_state,0);
    // Delete hook
    my_uc_hook_del(uc_stats, hk_stats, current_run_state);
    print_stats(current_run_state);
    print_checkpoints(current_run_state);
    my_uc_close(uc_stats,current_run_state);
...
void restore_from_checkpoint_timeit(current_run_state_t *current_run_state)
{
    ...
    my_uc_engine_setup(&uc_timingtest, current_run_state);
    ...
    //Run the program
    my_uc_hook_add("hook_placebo",uc_timingtest, &hk_placebo, UC_HOOK_CODE, hook_placebo, current_run_state,1,0); //HACKHACK
    ...
    my_uc_hook_del(uc_timingtest,hk_placebo,current_run_state); //HACKHACK
    my_uc_close(uc_timingtest,current_run_state);
}
...
void goldenrun_full_it (current_run_state_t* current_run_state)
{
    ...
    printf("\n~~~~ Running the program to display all instructions ~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
    uc_engine *uc_golden;
    my_uc_engine_setup(&uc_golden,current_run_state);

    current_run_state_reset(current_run_state); 
    current_run_state->run_mode=eGOLDENRUN_FULL_rm;
    /** For printing each line  - ADD the hook*/
    uc_hook hk_code_print_instructions; 
    my_uc_hook_add("hook_code_print_instructions",uc_golden, &hk_code_print_instructions, UC_HOOK_CODE, hook_code_print_instructions, current_run_state,1,0);
    my_uc_engine_start(uc_golden, current_run_state,0);     /** time one run **/
    /** For printing each line  - DELETE the hook*/
    my_uc_hook_del(uc_golden, hk_code_print_instructions,current_run_state);


    printf_output("Total instructions in faulting range:   %li\n",current_run_state->instruction_count);    
    print_outputs(uc_golden, current_run_state);
    my_uc_close(uc_golden,current_run_state);
}
...
void goldenrun_it(current_run_state_t* current_run_state)
{
    ...
    my_uc_hook_add("hook_code_print_fault_instructions",uc_golden, &hk_code_print_fault_instructions, UC_HOOK_CODE, hook_code_print_fault_instructions, current_run_state,1,0);

    my_uc_engine_start(uc_golden, current_run_state,0);  

    /** For printing each line in the faulting range - DELETE the hook */
    my_uc_hook_del(uc_golden, hk_code_print_fault_instructions,current_run_state);
    printf_output("Total instructions in faulting range:   %li\n",current_run_state->instruction_count);    
...
void memhook_it(current_run_state_t* current_run_state)
{
    ...
    my_uc_engine_setup(&uc_mem,current_run_state);

    current_run_state_reset(current_run_state); // reset the counter etc.
    //Print every line that runs
    my_uc_hook_add("hook_code_print_instructions",uc_mem, &hk_code_print_instructions, UC_HOOK_CODE, hook_code_print_instructions, current_run_state,1,0);
    /* ADD two memory hooks */
    my_uc_hook_add("hook_code_mem_write",uc_mem, &hk_mem_valid, UC_HOOK_MEM_WRITE, hook_mem_write, current_run_state,1,0);
    my_uc_hook_add("hook_code_mem_read_after",uc_mem, &hk_mem_valid, UC_HOOK_MEM_READ_AFTER, hook_mem_read_after, current_run_state,1,0);

    my_uc_engine_start(uc_mem, current_run_state,0);

    my_uc_hook_del(uc_mem, hk_code_print_instructions,current_run_state);
    my_uc_hook_del(uc_mem, hk_mem_valid,current_run_state);
    print_outputs(uc_mem, current_run_state);
    my_uc_close(uc_mem,current_run_state);
}
...
void *fault_it_thread(void *user_data)
{
   ...
   // printf_output("%li \n",workload.instruction);
        ...
        {
                fprintf(stderr, "Instruction to fault: %li is larger than the total number of instructions: %li !\n",
                    workload.instruction,
...
void stats_it(current_run_state_t* current_run_state)
{
    ...
    printf_output("Time of first run:                             %li ns \n", time_run);
    printf_output("Time to restore checkpoint:                    %li ns \n", time_checkpoint);
    printf_output("Time per instruction (approx):                 %li ns \n", time_run/num_instr);
    printf_output("Equivalent instructions to restore checkpoint: %li\n", time_checkpoint/(time_run/num_instr));
...
void debug_it(current_run_state_t* current_run_state)
{
    ...
    run_to_count_total_instructions(current_run_state);  
    uc_engine *uc_debug;
    my_uc_engine_setup(&uc_debug,current_run_state);
    current_run_state->run_mode=eDEBUG_rm;

    current_run_state_reset(current_run_state); 
    uc_hook hk_code_print_debug; 
    my_uc_hook_add("hook_code_print_instructions",uc_debug, &hk_code_print_debug, UC_HOOK_CODE, hook_code_print_debug, current_run_state,1,0);
    my_uc_engine_start(uc_debug, current_run_state,0);     /** time one run **/
    /** For printing each line  - DELETE the hook*/
    my_uc_hook_del(uc_debug, hk_code_print_debug,current_run_state);


    printf_output("Total instructions in faulting range:   %li\n",current_run_state->instruction_count);    
    print_outputs(uc_debug, current_run_state);
    my_uc_close(uc_debug,current_run_state);
}
...
{  
    ...
    while (current_fault !=NULL)
    {
        ...
        {
            fprintf(stderr,"No valid target found for fault at line: %li.\n",current_instruction_range_fault->instruction_start);
            my_exit(-1);
        }
        ...
        while (current_opcode_filter_fault !=NULL)
        {
            ...
            {
                operation_fault_t *current_operation_fault_list=current_lifespan_fault_list->operation_fault_head;
                while (current_operation_fault_list !=NULL)
                {
                    if (current_operation_fault_list->mask_count==0)
                    {
                    ...
                    {
                        count_running_total +=current_operation_fault_list->mask_count * count_targets;
                    }
                    current_operation_fault_list=current_operation_fault_list->next;
                }
...
void get_on_with_it(const char *code_buffer, const size_t code_buffer_size, current_run_state_t* current_run_state)
{
    ...
    fprintf(f, "\n\n\n\n##### Starting new run. ");
    fprintf(f, "Address: 0x%" PRIx64 ". Hit: %li. Lifespan: %li. ",fault_address,hit_count,current_run_state->fault_rule.lifespan.count);
    print_fault_rule_no_newline(f,&current_run_state->fault_rule);
    fprintf(f, " ###\n");


    uc_engine* uc; 
    my_uc_engine_setup(&uc,current_run_state);
    ...
    switch (current_run_state->run_state)
    {
        ...
        case HARD_STOP_rs:
            ...
            break;
        case ERRORED_rs:
            fprintf_output(f,"Run result: Fault errored program - last instruction %li.\n", current_run_state->instruction_count);
            print_outputs(uc, current_run_state);
            break;
        case TIMED_OUT_rs:
    ...
    {
        fprintf_output(f,"Instruction: %li. Checkpoint %li Time to run: %li ns\n ", 
        ...
        fprintf(f, "%li,%li,%li,%s,PLOTTINGLINE (for python graphs).\n ", 
            current_run_state->fault_rule.instruction,
            current_run_state->line_details_array[current_run_state->fault_rule.instruction].nearest_checkpoint, 
            current_run_state->time_to_run,
            run_state_to_string(current_run_state->run_state));
    }
    my_uc_close(uc,current_run_state);
}
                            ...
                            workload_t workload,current_run_state_t *current_run_state)
{
    ...
    if (current_instruction_range_fault !=NULL)
    {
        ...
        while (current_fault !=NULL)
        {
                ...
                case instruction_pointer_ft:
                    {
                        ...
                        while (current_opcode_filter_fault !=NULL)
                        {
                                        ...
                                        )
                            {
                                ...
                                {
                                    operation_fault_t *current_operation_fault_list=current_lifespan_fault->operation_fault_head;
                                    ...
                                    while (current_operation_fault_list !=NULL)
                                    {
                                        ...
                                        current_run_state->fault_rule.operation=current_operation_fault_list->operation;
                                        ...
                                        )
                                    {
                                        ...
                                        while (current_lifespan_fault!=NULL)
                                        {
                                            ...
                                            while (current_operation_fault_list !=NULL)
                                            {
                                                ...
                                                for (uint64_t mask_counter=0; mask_counter < current_operation_fault_list->mask_count; mask_counter++)
                                                {
                                                    current_run_state->fault_rule.number=register_counter;
                                                    current_run_state->fault_rule.opcode_filter_fault=current_opcode_filter_fault->string;
                                                    current_run_state->fault_rule.lifespan=current_lifespan_fault->lifespan;
                                                    current_run_state->fault_rule.operation=current_operation_fault_list->operation;
                                                    current_run_state->fault_rule.mask=current_operation_fault_list->masks[mask_counter];
            ...
            #ifdef DEBUGs
                printf_debug("Getting next fault instructions: %li\n",i);
            #endif
            current_fault=current_fault->next;
        }
    }
    if (current_run_state->stop_on_equivalence)
    {
        ...
        {
            #ifdef DEBUGs
                printf_debug("Freeing equivalence lists: %li\n",i);
            #endif
            // Free the memory 
