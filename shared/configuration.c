...
void get_patches(struct json_object* parsed_json)
{
    ...
    if (internal_binary_file_details.patches_count!=0)
    {
        ...
        for (uint64_t i=0;i<internal_binary_file_details.patches_count;i++)
        {
            ...
            if (temp == NULL)    
            {
                fprintf(stderr,"Error getting address for %li th patch.\n",i);
                my_exit(-1);  
            }
            ...
            if (temp == NULL)    
            {
                fprintf(stderr,"Error getting byte_array for %lith patch. Did you forget to include a 'byte array' tag?\n",i);
                my_exit(-1);  
            }
            ...
            {
                fprintf(stderr,"Error getting byte_array for %lith patch. byte_array is not an even number of hex digits.?\n",i);
                my_exit(-1);  
            }
...
void get_set_memory(struct json_object* parsed_json)
{
    ...
    for (uint64_t i=0;i<internal_binary_file_details.set_memory_count;i++)
    {
        ...
        if (temp == NULL)    
        {
            fprintf(stderr,"Error getting byte_array for %lith set memory. Did you forget to include a 'byte array' tag?\n",i);
            my_exit(-1);  
        }
        ...
        if (temp == NULL)    
        {
            fprintf(stderr,"Error getting json tag 'type' for %li th new input (Should be 'address' or 'sp offset').\n",i);
            my_exit(-1);  
        }
