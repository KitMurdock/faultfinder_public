...
void free_binary_file_details()
{
    ...
    {
        my_free(internal_binary_file_details.set_memory[i].byte_array, "internal binary file details - set memory - byte array");
    }
...
void get_memory_details(struct json_object *parsed_json)
{
    ...
    {
        fprintf(stdout, "Error. Main Memory is not large enough. Automatically adjusting\n");
        fprintf(stdout, " Main Memory size requested >>> 0x%" PRIx64 "\n", binary_file_details->memory_main.size);
        fprintf(stdout, " Binary File size actual >>> 0x%" PRIx64 "\n", internal_binary_file_details.code_buffer_size);
...
void get_memory_other_details(struct json_object* parsed_json)
{
    ...
    for (uint64_t i=0;i<internal_binary_file_details.memory_other_count;i++)
    {
        ...
        if (temp==NULL)    
        {
            fprintf(stderr,"Error getting address for %li th memory other.\n",i);
            my_exit(-1);  
        }
        ...
        if (temp==NULL)    
        {
            fprintf(stderr,"Error getting size  for %li th memory other.\n",i);
            my_exit(-1);  
        }
...
void get_stack_details(struct json_object *parsed_json)
{
    ...
    if (temp==NULL)    
    {
        fprintf(stdout, " >> No 'stack start' - automatically setting it to: 0x%" PRIx64 "\n",stack_end);
...
void get_skips(struct json_object* parsed_json)
{
    ...
    if (all_skips==NULL)    
    {
        fprintf(stderr,"Error getting instructions to skip.\n");
        my_exit(-1);     
    }
    ...
    for (uint64_t i=0;i<internal_binary_file_details.skips_count;i++)
    {
        ...
        if (temp==NULL)    
        {
            fprintf(stderr,"Error getting address for %li th skips.\n",i);
            my_exit(-1);  
        }
        ...
        if (temp==NULL)    
        {
            fprintf(stderr,"Error getting bytes value for %li th skip. Did you forget the 'bytes' tag?\n",i);
            my_exit(-1);  
        }
...
void get_hard_stops(struct json_object* parsed_json)
{
    ...
    for (uint64_t i=0;i<internal_binary_file_details.hard_stops_count;i++)
    {
        ...
        if (temp==NULL)    
        {
            fprintf(stderr,"Error getting address for %li th hard stop.\n",i);
            my_exit(-1);  
        }
        internal_binary_file_details.hard_stops[i]=strtol(json_object_get_string(temp),NULL,0);
    }
}
...
void get_set_memory(struct json_object* parsed_json)
{
    ...
    for (uint64_t i=0;i<internal_binary_file_details.set_memory_count;i++)
    {
        ...
        if (temp==NULL)    
        {
            fprintf(stderr,"Error getting format for %lith set memory. Did you forget to include a 'format' tag?\n",i);
            my_exit(-1);  
        }
        ...
        {
            fprintf(stderr,"No valid format set for %lith set memory. Options are 'hex' or 'ascii'\n",i);
            my_exit(-1);
        }

        // BYTE ARRAY & LENGTH
        json_object_object_get_ex(next_memory_item,"byte_array",&temp);
        if (temp==NULL)    
        {
            fprintf(stderr,"Error getting byte_array for %lith set memory. Did you forget to include a 'byte_array' tag?\n",i);
            my_exit(-1);  
        }
        ...
        if (internal_binary_file_details.set_memory[i].format==hex_format)
        {
            ...
            internal_binary_file_details.set_memory[i].byte_array=(uint8_t*) my_malloc ((internal_binary_file_details.set_memory[i].length+1),"internal binary file details - set memory - byte array");
        ...
        else 
        {
            ...
            // Checked - yes I do free this.
            internal_binary_file_details.set_memory[i].byte_array=my_malloc((sizeof(char))*(internal_binary_file_details.set_memory[i].length+1),"internal binary file details - set memory - byte array");
            // Checked - yes I do free this.
        ...
        if (temp==NULL)    
        {
            fprintf(stderr,"Error getting type for %li th new input (BTW should be 'address' or 'sp offset').\n",i);
            my_exit(-1);  
        }
        ...
        if (strcmp(json_object_get_string(temp),"address")==0)
        {
            ...
            if (temp==NULL)    
            {
                fprintf(stderr,"Error getting address for %li th new input. Did you include an 'address' tag?\n",i);
                my_exit(-1);  
            }
        ...
        else if (strcmp(json_object_get_string(temp),"sp_offset")==0)
        {
            ...
            if (temp==NULL)    
            {
                fprintf(stderr,"Error getting sp_offset for %li th new input. Did you include an 'sp_offset' tag?",i);
                my_exit(-1);  
            }
        ...
        {
            fprintf(stderr,"Error -  type for %li th new input (should be 'address' or 'sp_offset').\n",i);
            my_exit(-1);  
        }
...
void get_set_registers(struct json_object* parsed_json)
{
    ...
    for (uint64_t i=0;i<internal_binary_file_details.set_registers_count;i++)
    {
        ...
        if (temp==NULL)    
        {
            fprintf(stderr,"Error getting reg for %li th set registers. Use 'reg'.\n",i);
            my_exit(-1);  
        }
        const char* reg_name_temp=json_object_get_string(temp);
        printf("Getting register: %s \n", reg_name_temp);
        ...
        if (temp==NULL)    
        {
            fprintf(stderr,"Error getting reg value for %li th new register. Use 'reg value'\n",i);
            my_exit(-1);  
        }
...
void get_outputs(struct json_object* parsed_json)
{
    ...
    for (uint64_t i=0;i<internal_binary_file_details.outputs_count;i++)
    {
        ...
        if (temp==NULL)    
        {
            fprintf(stderr,"Error getting location for %lith output.\n",i);
            my_exit(-1);  
        }
        ...
        {
            fprintf(stderr,"No valid format set for %lith output. Received: %s\n",i,json_object_get_string(temp));
        ...
        if ( internal_binary_file_details.outputs[i].location==register_loc || internal_binary_file_details.outputs[i].location==address_in_register_loc)
        {
            ...
            if (temp==NULL)    
            {
                fprintf(stderr,"Error getting register for %lith output.\n",i);
                my_exit(-1);  
            }
        ...
        if ( internal_binary_file_details.outputs[i].location==relative_address_loc || internal_binary_file_details.outputs[i].location==fixed_address_loc)
        {
            ...
            if (temp==NULL)    
            {
                fprintf(stderr,"Error getting address for %lith output.\n",i);
                my_exit(-1);  
            }
        ...
        if (temp==NULL)    
        {
            fprintf(stderr,"Error getting length for %lith output.\n",i);
            my_exit(-1);  
        }
        ...
        if (temp==NULL)    
        {
            fprintf(stderr,"Error getting offest for %lith output.\n",i);
            my_exit(-1);  
        }
...
void load_configuration(const char* json_binary_filename)
{
  ...
  //  print_binary_file_details(); //TEMPTEMP
}
