...
void hook_code_start_address (uc_engine *uc, uint64_t address, uint64_t size, void *user_data)
{
    ...
    if (current_run_state->fault_rule.set==true)
    {
        ...
        add_hook_code_fault_address(uc, current_run_state, address_to_fault);
    }
...
{
    //printf("  <~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~> End address!!!  0x%" PRIx64 ". uc: %p\n",address,uc); //TEMPTEMP
...
void hook_code_skips(uc_engine *uc, uint64_t address, uint64_t size, void *user_data)
{
    ...
    if (current_run_state->run_mode==eGOLDENRUN_FULL_rm)
    {
        fprintf(current_run_state->file_fprintf,"Skipping address: 0x%" PRIx64 ". Size: %li\n",address,size);
    }
...
//    // print_all_registers(uc,stdout);
//     print_register_from_name(uc,stdout, "sp");
//     print_register_from_name(uc,stdout, "r12");
...
void hook_code_print_instructions(uc_engine *uc, uint64_t address, uint64_t size, void *user_data)
{
    ...
    //uint64_t count=current_run_state->instruction_count;
    ...
    if (!uc_mem_read(uc, address, tmp, size))
    {
        ...
        {
            fprintf(current_run_state->file_fprintf, "%08li ",current_run_state->instruction_count);
            if (current_run_state->address_hit_counter!=NULL && current_run_state->run_mode !=eCOUNT_INSTRUCTIONS_rm)
            {
                if (current_run_state->line_details_array!=NULL)
                {
                    // Do we ever get here??
                    printf ("MOOOOOO\n");
                   // printf ("hit:%04li. ", current_run_state->line_details_array[count].hit_count);
                }
                else
                {
                    printf ("hit:%04li. ", address_hit(current_run_state->address_hit_counter,address));
                }
            }
        }
        else
...
bool hook_mem_read_after(uc_engine *uc, uc_mem_type type, uint64_t address, uint64_t  size, uint64_t value, void *user_data)
{
    ...
    fprintf(current_run_state->file_fprintf, "\t\t\t\t\t\t\t\t\t\t\t\t >> Hooks: Mem Read. Address: 0x%" PRIx64 ". Size of read: %li Value: 0x%" PRIx64 " \n",address, size, value);
    return true;
}
...
bool hook_mem_write(uc_engine *uc, uc_mem_type type, uint64_t address, uint64_t  size, uint64_t value, void *user_data)
{
    ...
    current_run_state_t *current_run_state=(current_run_state_t *)user_data;
    fprintf(current_run_state->file_fprintf, "\t\t\t\t\t\t\t\t\t\t\t\t >> Hooks: Mem write. Address: 0x%" PRIx64 ". Size of write: %li Value: 0x%" PRIx64 " \n", address, size, value);
