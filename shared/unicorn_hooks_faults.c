...
uint64_t pc=0;
    ...
    current_run_state_t *current_run_state=(current_run_state_t *)user_data;
    uc_reg_read(uc,  binary_file_details->my_pc_reg,&pc);
    fprintf_output(current_run_state->file_fprintf, "Program Counter: 0x%" PRIx64 ". 16 bytes from PC: ",pc);
    uint8_t tmp[256];
    uc_mem_read(uc, pc, &tmp,16);
    phex_space(current_run_state->file_fprintf,tmp,16);
