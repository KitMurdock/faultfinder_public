...
{
    fprintf(current_run_state->file_fprintf, "****** Equivalence faults for instruction: %li ******\n", instruction);
...
void save_context_to_file(uc_engine *uc, const char *filename)
{
    struct uc_context_copy
    {
        size_t size;
...
{
    // Yes this is freed!
    uint8_t *my_bytes = my_malloc(binary_file_details->stack.size, "my_bytes - save stack to file");
    ...
    fclose(fp);
    my_free(my_bytes, "my_bytes - save stack to file");
}
...
void get_lifespan_fault_from_line(const char *original_line, lifespan_t *l)
{
    char *line = MY_STACK_ALLOC(strlen(original_line) + 1);
...
run_list_t *parse(const char *filename)
{
    ...
    operation_fault_t *current_operation_fault_list = NULL;
    ...
    while ((num_chars_read = getline(&lineptr, &line_len, fp)) != -1)
    {
        ...
        if (strlen(line) > 0 && line[0] != '#')
        {
            ...
            else if ((strncmp(line, "Registers:", 10) == 0) || (strncmp(line, "Registers-force:", 15) == 0) || (strncmp(line, "Instruction:", 11) == 0) || (strncmp(line, "Flags:", 6) == 0) || (strncmp(line, "Instruction Pointer:", 20) == 0))
            {
                ...
                current_operation_fault_list = NULL;
            ...
            else if (strncmp(line, "Operation", 9) == 0)
            {
                ...
                if (current_operation_fault_list != NULL)
                {
                    // Attach us to the end of the chain
                    current_operation_fault_list->next = new_operation_list;
                }
                else
                {
                    // This is the first operation_list, set us as the head.
                ...
                current_operation_fault_list = new_operation_list;
            }
            else if (strncmp(line, "Masks:", 6) == 0)
            {
                ...
                current_operation_fault_list->mask_count = get_masks_from_line(mask_masks, &(current_operation_fault_list->masks));
            }
        }
    }
    return run_list;
    ...
    while (current_instruction_range_fault != NULL)
    {
        ...
        while (current_target_fault != NULL)
        {
            ...
            while (current_opcode_filter_fault != NULL)
            {
                ...
                {
                    // FAULT LIFECYCLE
                    fprintf(f, "\tLifespan: %li ", current_lifespan_fault->lifespan.count);
