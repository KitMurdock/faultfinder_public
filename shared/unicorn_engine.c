...
{
    uc_err err=0;
    // printf_OTTER("Restore. Context pointer: %p. Size: %li\n",the_context,context_size(the_context));
...
void uc_restore_from_checkpoint(uc_engine *uc, current_run_state_t *current_run_state, uint64_t instr)
{
    ...
    */
    ...
    fprintf_output( current_run_state->file_fprintf,
                    "Restoring check point. Starting from: %li Address: 0x%" PRIx64 " . (Checkpoint: %li Address: 0x%" PRIx64 ") \n", 
                    instr, 
                    address,
                    checkpoint_instr,
...
void uc_engine_set_memory_inputs(uc_engine *uc, current_run_state_t *current_run_state)
{
    ...
    for (uint64_t i=0; i < binary_file_details->set_memory_count; i++)
    {
        if (binary_file_details->set_memory[i].type == address_memory)
        {
            ...
            fprintf_output(fd, "Writing memory input #%li directly to address: 0x%016lx: ", i,new_address);
...
void current_run_state_setup(current_run_state_t* current_run_state)
{
#ifdef DEBUG
    printf_debug("current_run_state_setup - malloc the hooks for hard stops and skips\n");
#endif
...
void current_run_state_teardown(current_run_state_t* current_run_state)
{
#ifdef DEBUG
    printf_debug("current_run_state_teardown - malloc the hooks for hard stops and skips\n");
#endif
...
{
    // printf_OTTER("uc_mem_map: Address: 0x%" PRIx64 ",  size: 0x%" PRIx64 " %s \n", address,size,desc);
    #ifdef DEBUG
...
void uc_engine_open_and_mem_map(uc_engine **uc,current_run_state_t* current_run_state,char* desc)
{
    ...
    // printf_OTTER("OPEN OPEN OPEN: %s %p\n",desc,*uc);
    ...
    if (err != UC_ERR_OK)
    {
        ...
        fprintf(stderr, "Ignoring error and attempting to continue anyway.\n");
     //   my_exit(-1);
    }
...
void my_uc_engine_start(uc_engine *uc, current_run_state_t *current_run_state, uint64_t max_instructions)
{
    ...
    /*  Run it
        ...
        that tells me that it made it to the end! And then calls uc_emu_stop  
        (I can't know the address of the instruction AFTER because I don't know how big the last instruction is!
    */
...
void uc_unmap_all_memory(uc_engine *uc,current_run_state_t* current_run_state)
{
    ...
    // printf_OTTER("uc_mem_unmap. 0x%lx\n",binary_file_details->memory_main.address);
