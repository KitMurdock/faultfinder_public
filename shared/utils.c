...
void *my_malloc (size_t s, char* description)
{
    ...
    malloc_counter++;
    #ifdef SHOW_MALLOC
        printf_debug("Description: %s. Pointer: %p. %05i. 1. MY_MALLOC.  Size: %ld\n",description,ptr,mem_counter,s);    
        #endif
    return ptr;
}
...
void print_current_run_state(current_run_state_t* c)
{
    ...
    printf("Total instruction count:     %li\n",c->total_instruction_count);
    printf("Number of checkpoints:       %li\n",c->total_num_checkpoints);
    ...
    printf("Time to run:                 %li\n",c->time_to_run);
    printf("Time to restore checkpoint:  %li\n",c->time_to_restore_checkpoint);
    printf("Equivalence count:           %li\n",c->equivalence_count);
    printf("Checkpoints: \n");
    for (uint64_t i=1 ; i<c->total_instruction_count+1;i++)
    {
        if (c->line_details_array[i].checkpoint==true)
        {
            printf(" > Instruction: %07li Address: 0x%" PRIx64 " Hitcount: %li\n",
...
{
    fprintf(fd,"Instruction: %08li. Faulting Target: ",fault_rule->instruction);
...
uint64_t fault_instruction_reg (op_t fault_op, uint64_t tmp,uint64_t size)
{
    switch (fault_op)
    {   
        case eSKIP_op:
