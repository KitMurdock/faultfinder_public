...
{
    printf_output("Saving a checkpoint: 0x%" PRIx64 ". Count: %li\n",address,num);

    /// Save the context (registers) to the array
    uc_err err=uc_context_alloc(uc, &current_run_state->line_details_array[num].the_context);
    ...
    err=uc_context_save(uc, current_run_state->line_details_array[num].the_context);
    if (err !=UC_ERR_OK)
    {
    ...
    //TODO - check free
    current_run_state->line_details_array[num].stack=my_malloc(sizeof(uint64_t)*binary_file_details->stack.size,"line_details_array - stack");
    err=uc_mem_read(uc, binary_file_details->stack.address, current_run_state->line_details_array[num].stack, binary_file_details->stack.size);
    if (err !=UC_ERR_OK)
    {
        fprintf(stderr,"Unable to read and save the stack %u: %s\n", err, uc_strerror(err));
        my_exit(-1);
    }
    current_run_state->line_details_array[num].memory_main=my_malloc(sizeof(uint64_t)*binary_file_details->memory_main.size,"line_details_array - memory main");
    ...
    current_run_state->line_details_array[num].memory_other=my_malloc (sizeof(uint64_t)*binary_file_details->memory_other_count,"line_details_array - memory other pointer");
    for (uint64_t i=0;i<binary_file_details->memory_other_count;i++)
    {
        //save the other memory bits
        current_run_state->line_details_array[num].memory_other[i]=my_malloc (sizeof(uint64_t)* binary_file_details->memory_other[i].size, "line_details_array - memory other");
    ...
    // print_context_and_stack(current_run_state->line_details_array,num);  //TEMPTEMP
...
void hook_code_stats(uc_engine *uc, uint64_t address, uint64_t size, void *user_data)
{
    ...
    if (size==2  && (binary_file_details->my_uc_arch==UC_ARCH_ARM    || binary_file_details->my_uc_arch==UC_ARCH_ARM64)) 
    {
        ...
        else
        {
            ...
            if (!uc_mem_read(uc, address, tmp, size)) 
            {
                ...
                if (count >0)
                {
                    ...
                    snprintf(current_run_state->line_details_array[num].op_mnemonic,50,"%s",insn[0].mnemonic);
                    
                    // Get the text from the op code - to see which less aregisters are in there.
                    ...
                    {
                        reg=register_name_from_int(i); //SAVESAVE  //CHECKCHECK (memory allocation??)
